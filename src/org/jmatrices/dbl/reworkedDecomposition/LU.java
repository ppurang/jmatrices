 ...
 * ------------------------ */
    ...
    public LU(Matrix a) {   //renamed A to a
        ...
        for (int j = 1; j <= n; j++) {
            ...
            for (int i = 1; i <= m; i++) {
                System.out.println("\t\t Setting value for LUCol"+ j+ "[" + i +",1]  " +  LU.getValue(i, j));
                LUcolj.setValue(i, 1, LU.getValue(i, j));
            }
            System.out.println("\t\t LUCol"+ j+ " =  " +  LUcolj);
            ...
            for (int i = 1; i <= m; i++) {
                ...
                System.out.println("\t\t\t Setting value for LURow"+ i+ " =   " +  LUrowi);

                // Most of the time is spent in the following dot product.

                int kmax = Math.min(i, j);
                double s = 0.0;
                System.out.println("\t\t\t Starting Loop (int k = 1; k <= "+ kmax+"; k++) ");
                for (int k = 1; k < kmax; k++) {
                    //s += LUrowi[k] * LUcolj[k];

                    System.out.println("\t\t\t\t calculating s using s = " + s + ", LUrowi(1,"+k+") = "+ LUrowi.getValue(1, k) +", LUcolj("+k+",1) = " + LUcolj.getValue(k, 1) );
                    s = s + LUrowi.getValue(1, k) * LUcolj.getValue(k, 1);
                    System.out.println("\t\t\t\t s = "+ s);
                }

                //LUrowi[j] = LUcolj[i] -= s;
                System.out.println("\t\t@@Using LUcolj.getValue(i, 1) " + LUcolj.getValue(i, 1) + ", s = " + s );
                LUcolj.setValue(i, 1, LUcolj.getValue(i, 1) - s);
                System.out.println("\t\t*** Setting value for LUCol"+ j+ "[" + i +",1]" +  (LUcolj.getValue(i, 1) - s));

                System.out.println("\t\t@@Using LUcolj.getValue(i, 1) " + LUcolj.getValue(i, 1));
                LUrowi.setValue(1, j, LUcolj.getValue(i, 1));
                System.out.println("\t\t*** Setting value for LURow"+ j+ " =   " +  LUcolj.getValue(i, 1));
            ...
            int p = j;
            System.out.println("\t Starting Loop (int i = "+j+"; i < "+ m+"; i++) ");
            for (int i = j+1; i <= m; i++) {
                System.out.println("comparing LUcolj.getValue("+i+", 1) = "+ LUcolj.getValue(i, 1) +" > LUcolj.getValue("+p+", 1) =" + LUcolj.getValue(p, 1));
            ...
            if (p != j) {
                System.out.println("\t\t Starting Loop (int k = "+1+"; k <= "+ n+"; k++) ");
                for (int k = 1; k <= n; k++) {
                    double t = LU.getValue(p, k);
                    System.out.println("\t\t Setting  LU["+ p + "][" + k + "] " + LU.getValue(j, k));
                ...
                System.out.println("\t\t piv["+p+"] ("+piv.getValue(p,1)+") = piv["+j+"] ("+piv.getValue(j,1)+") ");
                piv.setValue(j,1,k);
            ...
            // Compute multipliers.
            System.out.println("\t Evaluating if(j < m & LU.getValue(j, j) != 0.0)" + (j <= m & LU.getValue(j, j) != 0.0));
            if (j <= m & LU.getValue(j, j) != 0.0) {
                System.out.println("\t\t Starting Loop (int i = "+(j+1)+"; i <= "+ m+"; i++) ");
                for (int i = j + 1; i <= m; i++) {
                    System.out.println("\t\t\tSetting  LU.setValue("+i+"," +j+") = " + (LU.getValue(i, j) / LU.getValue(j, j)));
 ...
 * ------------------------ */
    ...
    public Matrix solve(Matrix b) {
        ...
        for (int k = 1; k <= n; k++) {
            for (int i = k + 1; i <= n; i++) {
                for (int j = 1; j <= nx; j++) {
                    System.out.println("setting X["+i+"]["+j+"] = " + ( X.getValue(i, j) - (X.getValue(k, j) * LU.getValue(i, k))));
        ...
        for (int k = n; k >= 1; k--) {
            ...
            for (int j = 1; j <= nx; j++) {
                ...
                System.out.println("\t\tsetting X["+k+"]["+j+"] = " + (X.getValue(k, j) / LU.getValue(k, k)));
