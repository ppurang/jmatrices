 ...
 * Time: 16:06:06
 */
public class LightMatrixImpl implements Matrix {
    ...
    public LightMatrixImpl(int rows, int cols, double[][] elems) {
        this(rows, cols);
        populateElements(elems);
    }

    protected void populateElements(double[][] elems) {
        //todo - performance hit but better management of code - any better solution?!
        if (checkConformity(elems)) {
            for (int i = 0; i < elems.length; i++) {
                for (int j = 0; j < elems[i].length; j++) {
                    set(i + 1, j + 1, elems[i][j]);
                }
            }
        } else
    ...
    protected boolean checkConformity(double[][] elems) {
        return checkConformity(elems, rows, cols);   // presumes that rows and cols have been initialized to their values
    }

    protected boolean checkConformity(double[][] elems, int rows, int cols) {
